...
const Options = ({ stream }) => {
    ...
    const handleMic = () => {
        ...
        if (stream && stream.srcObject) stream.srcObject.getTracks().forEach(function (track) {
    ...
    return (
        <>
            <Box sx={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#000d1a" }} >
                <Container component='main' maxWidth='xs' sx={{ pb: 2 }} >
                    <div className="wrapper_ option_wrapper d-flex j-c-s-b">
                        {optionGrout.map((item, index) => {
                            return <IconButton key={index} onClick={() => item.fun()} color="secondary">
                                {item.icon}
                            </IconButton>
                        })}
            ...
            <div className="snackbar_container">
                <Snackbar
                    ...
                    message={`copy code : http://..${roomID.substr(0, 8)}...`}
                />
            </div>
